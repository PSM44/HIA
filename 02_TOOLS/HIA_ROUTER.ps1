...
function Show-HIAHelp {
    ...
    Write-Host "  ai show-policy"
    Write-Host "      Muestra MODEL.ROUTING.REGISTRY.json y proveedores disponibles."
    Write-Host "  ai route reasoning [prompt]"
    Write-Host "      Enruta una tarea de razonamiento general."
    Write-Host "  ai route code [prompt]"
    Write-Host "      Enruta una tarea de cambio de codigo como CODE_CHANGE."
    Write-Host "  ai route local_tool [prompt]"
    Write-Host "      Enruta una tarea local permitida por allowlist (default: git status)."
    Write-Host "  ai route <TASK_TYPE> [prompt]"
    Write-Host "      Enruta por TaskType explicito usando MODEL.ROUTING.REGISTRY.json."
    Write-Host "  ai dispatch <TASK_TYPE> [risk]"
    Write-Host "      Despacha una tarea segun politica AI dispatch."
    Write-Host "  ai prompt <tool> <tasktype> [risk] [--json]"
    Write-Host "      Genera prompt operativo para una herramienta AI."
    Write-Host "  ai plan <PROJECT_ID> <request> [--remember] [--preset readiness|next-step|risk-scan]"
    Write-Host "      Crea un plan AI read-only para un proyecto."
    Write-Host "  ai memory <add|get> <PROJECT_ID> [note]"
    Write-Host "      Gestiona memoria AI por proyecto."
    Write-Host ""
