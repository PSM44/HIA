...
function Get-HIABatonValueByHeaders {
    ...
    foreach ($h in $Headers) {
        if ($h -match "NEXT_ACTION|PROXIMA_ACCION|SIGUIENTE_ACCION") {
    ...
    # PRJPB_FIX_005:
    # NEXT_ACTION must mean "next executable work", not "first historical baton item".
    # Therefore, when asking for NEXT_ACTION, accept only pipe-delimited baton lines
    # whose final field is exactly "ready". Ignore done/backlog/fixed/closed/etc.
    if ($isNextActionLookup) {
        foreach ($line in $lines) {
            ...
            if ($trimmed -notmatch "^PRJPB[_A-Z0-9]*\s*\|") {
                continue
            }

            $parts = @($trimmed -split "\|" | ForEach-Object { <#
===============================================================================
MODULE: HIA_PROJECT_ENGINE.ps1
SYSTEM: HIA - Human Intelligence Amplifier
TYPE: PROJECT ENGINE
...
.Trim() })
            ...
            $status = $parts[-1].ToLowerInvariant()
            if ($status -eq "ready") {
                return $trimmed
            }
        }
    ...
    foreach ($header in $Headers) {
        ...
        foreach ($line in $lines) {
            ...
            if (-not $foundHeader) {
                if ($trimmed -eq $header -or $trimmed -like ("*" + $header + "*")) {
                    $foundHeader = $true
                }
                continue
            }
